...
from .models.shared import MenuInput
router = APIRouter()


@router.get("/files/")
async def get_files_for_menu(input: MenuInput):
    """
    ...
    if input.language == "multi":
        ...
        bind_vars = {"language": input.language}
    query_result = execute_query(menu_query, bind_vars)
    return {"results": query_result.result}


@router.get("/filter/")
async def get_files_for_filter_menu(input: MenuInput):
    """
    Given a language, return list of files for the category menu
    """
    query_result = execute_query(
        menu_queries.QUERY_FILES_FOR_CATEGORY, {"language": input.language}
    )
    return {"filteritems": query_result.result}


@router.get("/category/")
async def get_categories_for_filter_menu(input: MenuInput):
    """
...
@router.get("/sidebar/")
async def get_data_for_sidebar_menu(input: MenuInput):
    """
    ...
    if input.language == "multi":
        ...
        current_bind_vars = {"language": input.language}
