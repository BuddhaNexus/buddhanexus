...
def load_metadata_from_files(paths: List[str], db: StandardDatabase) -> None:
    ...
    for path in paths:
        try:
            ...
            df['filename'] = df['filename'].apply(get_filename_from_segmentnr) # this is to remove duplicate parts of the same file            
            df = df.drop_duplicates(subset=["filename"])
            df = df[["filename", "displayName", "category", "collection", "textname"]] # metadata might contain more data; we are only interested in these columns
